            ...
            for (int i = 0; i < instructionAmount; i++) {
                ...
                if (remove) {
                    if (!inserted.isEmpty()) {
                        ...
                        if (ourPQ.remove(toRem) != pq.remove(toRem)) {
                            System.out.println("Fail: Point - 1");
                            return false;
                        }
                    }
                } else {
                ...
                if (ourPQ.isEmpty() != pq.isEmpty()) {
                    System.out.println("Fail: Point - 2");
                    return false;
                }
                if (ourPQ.size() != pq.size()) {
                    System.out.println("Fail: Point - 3");
                    return false;
                }
                if (!ourPQ.isEmpty() && !ourPQ.peek().equals(pq.peek())) {
                    System.out.println("Fail: Point - 4");
                    return false;
                }
            }
            while (!pq.isEmpty()) {
                if (ourPQ.isEmpty() != pq.isEmpty()) {
                    System.out.println("Fail: Point - 5");
                    return false;
                }
                if (ourPQ.size() != pq.size()) {
                    System.out.println("Fail: Point - 6");
                    return false;
                }
                if (!ourPQ.isEmpty() && !ourPQ.peek().equals(pq.peek())) {
                    System.out.println("Fail: Point - 7");
                    return false;
                }
                if (!ourPQ.poll().equals(pq.poll())) {
                    System.out.println("Fail: Point - 8");
                    return false;
                }
            }
            if (!pq.isEmpty() || !ourPQ.isEmpty()) {
                System.out.println("Fail: Point - 9");
                return false;
            }
        }
        return true;
